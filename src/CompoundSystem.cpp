...
class RigidUnit {
    ...
    const void Print() const {
        std::cout << "unit" << " "
                  ...
                  << "inBoDih " << inboardBondDihedralAngle << " " << std::endl;
    ...
    void PrintTransforms() const {
        ...
        SimTK::Test::PrintTransform(frameInParentFrameRotated,
                                    6,
                                    "frameInParentFrameRotated",
                                    "frameInParentFrameRotated");
    }
};
...
void buildUpRigidBody(Compound::AtomIndex cAIx,
    ...
    int numberOfNonBendyBondsOnThisAtom =
        atomBonding.treeBondedCAIxs.size() - atomBonding.freeTreeBondedCAIxs.size();
    if (numberOfNonBendyBondsOnThisAtom == 0) {
        return;
    }
    ...
    for (atomBondingIt = atomBonding.treeBondedCAIxs.begin();
        ...
        if ((bond.getMobility() == BondMobility::Translation)
            || (bond.getMobility() == BondMobility::FreeLine) || (bond.getMobility() == BondMobility::Free)
            ...
            || (bond.getMobility() == BondMobility::Torsion) || (bond.getMobility() == BondMobility::AnglePin)
            || (bond.getMobility() == BondMobility::BendStretch)
            || (bond.getMobility() == BondMobility::Slider) || (bond.getMobility() == BondMobility::Cylinder)
            || (bond.getMobility() == BondMobility::Spherical)
            || (bond.getMobility() == BondMobility::OrthoSpherical)
            || (bond.getMobility() == BondMobility::BallF) || (bond.getMobility() == BondMobility::BallM)) {
            ...
            int numberOfNonBendyBondsOnBondedAtom =
                bondedAtomBonding.treeBondedCAIxs.size() - bondedAtomBonding.freeTreeBondedCAIxs.size();
            ...
            buildUpRigidBody(*atomBondingIt, clusterIx, clusterAIxs, atomBondings, compoundRep);
        }
 ...
 * X_parentBC_childBC
 * --> */
CompoundSystem& CompoundSystem::calc_XPF_XBM_new(SimTK::Transform& Fr_X_Mr,
    ...
    //    - X_PF is parent rigid unit inboard_BC to the outboard
    //       atom's BC (parent BC) with the Y axis switched to Z axis
    //    - X_MB is the inboard bond parent BC to child BC transform
 ...
 * transforms.
 * --> */
CompoundSystem& CompoundSystem::calc_XPF_XBM(SimTK::Compound& compound,
    ...
    //      with the Z axis switched to Y
        ...
        PFBM[1] = X_childBC_parentBC * XAxis_To_ZAxis * XAxis_To_YAxis;
        PFBM[0] = oldX_PB * PFBM[1];
    }
...
#pragma region MODEL_1
    ...
    if (showDebugMessages) {
        cout << "Step 1 create atomBonds" << endl;
    }
...
#pragma endregion MODEL_1
    ...
    if (showDebugMessages) {
        cout << "Step 2 analyze bonding structure" << endl;
    }
    ...
    for (Compound::BondIndex bIxCnt(0); bIxCnt < compound.getNumBonds(); ++bIxCnt) {
        ...
        // Add bond to DuMM
        dumm.addBond(atomBondings[cAIx_0].dummAIx, atomBondings[cAIx_1].dummAIx);
    ...
    if (showDebugMessages) {
        cout << "Step 3 distribute atoms" << endl;
    }
    ...
    for (atomBondIt = atomBondings.begin(); atomBondIt != atomBondings.end(); ++atomBondIt) {
        ...
        if (atomBonding.clusterIx.isValid()) {
            continue;
        }
        ...
        buildUpRigidBody(cAIx, clusterIx, rigidUnit.clusterCAIxs, atomBondings, compoundRep);
    }
    ...
    if (showDebugMessages) {
        cout << "Step 4 assign rigid body parents" << endl;
    }
    ...
    for (Compound::BondIndex bIxCnt(0); bIxCnt < compound.getNumBonds(); ++bIxCnt) {
        ...
        if (bond.getMobility() == BondMobility::Rigid) {
            ...
            Transform T_X_Mr = compoundRep.calcDefaultBondCenterFrameInCompoundFrame(
                compoundRep.getBondCenterInfo(childUnit.inboardBCIx),
                atomFrameCache);
    ...
    for (rigidUnitI = rigidUnits.begin(); rigidUnitI != rigidUnits.end(); ++rigidUnitI) {
        ...
        if (rigidUnit.parentClusterIx.isValid()) {
            continue;
        }
    ...
    if (showDebugMessages) {
        cout << "Step 6 set child frames" << endl;
    }
    ...
    if (showDebugMessages) {
        cout << "Step 7 populate dumm clusters" << endl;
    }
    ...
    if (showDebugMessages) {
        cout << "Step 8 create mobilized bodies" << endl;
    }
    ...
    for (rigidUnitI = rigidUnits.begin(); rigidUnitI != rigidUnits.end(); ++rigidUnitI) {
        ...
        if (!unit.parentClusterIx.isValid()) {
            ...
            SimTK::MassProperties massProps = dumm.calcClusterMassProperties(unit.clusterIx);

            // std::cout << "[INERTIA CompoundSystem::modelOneCompound] " << massProps.getInertia() <<
            // std::endl;
        ...
        else // Unit has a parent body
        {
            ...
            Bond& unitInboardBond = compoundRep.updBond(compoundRep.updBondInfo(unit.inboardBondIx));
            ...
            // const Bond chemBond = bondInfo.getBond();
            const Bond& chemBond = compoundRep.getBond(bondInfo);

            // Get frame in parent rigid unit frame = Fr_X_M0
            Transform Fr_X_M0 = unit.frameInParentFrame;

            Transform Fr_X_Mr = unit.frameInParentFrameRotated;

            Transform X_parentBC_childBC = chemBond.getDefaultBondCenterFrameInOtherBondCenterFrame();
            ...
            if (unitInboardBond.getMobility() == BondMobility::OrthoSpherical) {
                Transform X_parentAtom_BC =
                    compound.calcDefaultBondCenterFrameInParentAtomFrame(parentAtomId, originAtomId);
                Transform X_childAtom_BC =
                    compound.calcDefaultBondCenterFrameInChildAtomFrame(parentAtomId, originAtomId);
            ...
            if (testRiboseMobilizer) {
                ...
                if (unitInboardBond.getMobility() == BondMobility::Torsion) {
                    MobilizedBody::Pin torsionBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::Torsion anglePinBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::BendStretch bendStretchBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::Slider sliderBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::Ball ballBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::SphericalCoords sphereBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::Universal universalMBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::Translation transBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::FreeLine freeLineBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::LineOrientation lineOrientationFBody(parentMobod,
                                                                        PFBM[0],
                                                                        massProps,
                                                                        PFBM[1]);
                    ...
                    MobilizedBody::LineOrientation lineOrientationMBody(parentMobod,
                                                                        PFBM[0],
                                                                        massProps,
                                                                        PFBM[1]);
                    ...
                    MobilizedBody::Free freeBody(parentMobod, PFBM[0], massProps, PFBM[1]);
                    ...
                    MobilizedBody::Cylinder cylinderBody(parentMobod, PFBM[0], massProps, PFBM[1]);
